<script lang="ts">
  ...
  let formVals = {
    ...
    liveQuestion:
      $entities.appControls.QUESTION ||
      ({
        controlType: 'QUESTION',
        imageUrl:
          'https://images.fineartamerica.com/images/artworkimages/mediumlarge/2/1-single-blue-question-mark-jesper-klausenscience-photo-library.jpg',
        question: 'Spoons or forks?',
        isActive: false,
      } as AppControlQuestion),
    embed:
      $entities.appControls.EMBED ||
      ({
        embedUrl: '',
        instructions: '',
        isActive: false,
...
<form on:submit|preventDefault={updateSettings}>
  ...
  <div>
    <div>
      <strong> Get the audience involved. </strong>
    </div>
    <div class="activity-ctas">
      <button class="secondary" type="button" on:click={setActivity('QUESTION')}
        >Ask a question</button
      >
      <button class="secondary" type="button" on:click={setActivity('EMBED')}
        >Embed an activity</button
      >
      {#if !!(formVals.liveQuestion.isActive || formVals.embed.isActive)}
        <button class="secondary" type="button" on:click={setActivity(null)}
  ...
  {#if formVals.liveQuestion.isActive}
    <label>
      <figure class="question-image">
        <img src={formVals.liveQuestion.imageUrl} />
      </figure>
      Add an image
      <input
        type="text"
        name="imageUrl"
        bind:value={formVals.liveQuestion.imageUrl}
      />
    </label>
    <label>
      ...
      <input
        type="text"
        name="question"
        bind:value={formVals.liveQuestion.question}
      />
    </label>
  {/if}
  ...
  {#if formVals.embed.isActive}
    ...
    <label>
      ...
      <input
        name="embedUrl"
        bind:value={formVals.embed.embedUrl}
        placeholder="<iframe ... />"
      />
    </label>
